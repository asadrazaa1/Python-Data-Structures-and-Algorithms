from SearchTrees.treeMap import TreeMap
...
class RedBlackTreeMap(TreeMap):
    ...
    class _Node(TreeMap._Node):
        """Node class for reb black tree maintains bit that denotes color"""
        __slots__ = '_red'  # addition data member to the node class
    ...
    def _resolve_red(self, p):
        if self.is_root(p):
            self._set_black(p)      # makes root black
        else:
    ...
    def _rebalance_delete(self, p):
        if len(self) == 1:
            ...
            if n == 1:                          # deficit exists unless child is a leaf node
                c = next(self.children(p))
                if not self._is_red_leaf(c):
                    self._fix_deficit(p, c)
            elif n == 2:                        # remove black node with red child
