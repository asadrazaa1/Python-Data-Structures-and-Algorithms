...
def quick_select(S, k):
    """Return the kth the smallest element of list S, for k from 1 to len(S)"""
    if len(S) == 1:
        return S[0]
    pivot = random.choice(S)        # pick random pivot element from S
    L = [x for x in S if x < pivot]     # elements less than pivot
    E = [x for x in S if x == pivot]     # elements equal pivot
    ...
    if k <= len(L):
        ...
        return quick_select(G, k)       # kth smallest is jth in G
