...
class BinaryEulerTour(EulerTour):
    """Abstract base class for performaing euler tour of a binary tree
    
    This versio includes an additional _hook_invisit that is called after the 
    tour of the left subtree(if any), yet before the tour of the right subtree(if any)
    
    Note: Right child is always assigned index 1 in path, even if no left sibling
    """
    def _tour(self, p, d, path):
        return super()._tour(p, d, path)